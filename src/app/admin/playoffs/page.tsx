...
import { generatePlayoffFixturesAction, getPlayoffFixturesAction, clearPlayoffFixturesAction } from './actions';
...
export default function ManagePlayoffsPage() {
  ...
  const handleGenerateFixtures = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: "Error en Generación", description: result.message, variant: "destructive" });
      }
    } catch (err) {
  ...
  const handleClearFixtures = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: "Error al Limpiar", description: result.message, variant: "destructive" });
      }
    } catch (err) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[calc(100vh-288px)]">
        ...
        <p className="text-xl text-muted-foreground">Cargando datos de playoffs...</p>
      </div>
    );
  }

  if (error && !isGenerating && !isClearing) { // Only show main error if not during an action
    return (
      <div className="flex flex-col justify-center items-center min-h-[calc(100vh-288px)] text-center p-4">
        <AlertTriangle className="h-16 w-16 text-destructive mb-4" />
        <p className="text-xl text-destructive font-semibold">Error al Cargar Llaves de Playoffs</p>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <SectionTitle>Gestionar Playoffs</SectionTitle>
